...
class Custom_Uploader_Attachment_Fields {
	...
	/**
	 * Flag indicating if Google URL Shortener is active.
	 *
	 * @since    1.0.0
	 * @access   private
	 * @var      bool    $google_url_shortener_is_enabled    Currect state of Google URL Shortener.
	 */
	private $google_url_shortener_is_enabled;
	...
	public function add_fields_to_edit( $form_fields, $post ) {
		...
		if($this->google_url_shortener_is_enabled) {
	...
	public function edit_fields( $attachment_id ) {
	    ...
	    if ( $this->google_url_shortener_is_enabled && isset( $_REQUEST['attachments'][$attachment_id]['cup_google_short_url_id'] ) ) {
	        $cup_date_taken = $_REQUEST['attachments'][$attachment_id]['cup_google_short_url_id'];
	        update_post_meta( $attachment_id, 'cup_google_short_url_id', $cup_date_taken );
	    }
